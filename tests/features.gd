...
func run() -> void:
	...
	place(b, Vector3(10, 0.1, 0))
	await settle()
	game.combat.explode(Vector3(0, 0.2, 0), 1)
	check(b.hp < 100 and b.hp > 0, "blast reaches beyond the former eight-metre radius")
	check(game.burn_zones.zones.size() == 1, "frag creates an authoritative burn zone")
	place(b, Vector3(15, 0.1, 0))
	await settle()
	game.combat.explode(Vector3(0, 0.2, 0), 1)
	check(b.hp == 100, "blast stops outside fourteen metres")
	game.burn_zones.clear()
	game.burn_zones.ignite(Vector3(0, 0.2, 0), 1)
	place(b, Vector3(3, 0.1, 0))
	await settle()
	game.burn_zones.advance(1)
	check(is_equal_approx(b.hp, 82), "burn deals eighteen HP per second in four ticks")
	b.team = a.team
	game.burn_zones.advance(0.5)
	check(is_equal_approx(b.hp, 82), "burn preserves team damage immunity")
	b.team = 1
	b.protection = 1
	game.burn_zones.advance(0.5)
	check(is_equal_approx(b.hp, 82), "burn respects spawn protection")
	b.protection = 0
	b.position.y = 3.7
	game.burn_zones.advance(0.5)
	check(is_equal_approx(b.hp, 82), "ground fire does not reach another storey")
	var rows: Array = game.burn_zones.snapshot()
	game.burn_zones.advance(3.5)
	check(game.burn_zones.zones.is_empty(), "burn ends after six seconds")
	game.is_host = false
	game.burn_zones.sync(rows)
	check(game.burn_zones.zones.size() == 1 and is_equal_approx(game.burn_zones.zones.values()[0].left, 3.5), "late snapshot retains remaining burn lifetime")
