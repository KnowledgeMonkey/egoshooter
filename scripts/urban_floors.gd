...
static func build(target: Node3D, x: float, z: float, tint: Color) -> void:
	var floors := UrbanFloors.new()
	floors.arena = target
	floors.origin = Vector3(x, 0, z)
	floors.accent = tint.lerp(Color("dad8cb"), 0.36)
...
func _wall(axis: int, fixed: float, width: float, openings: Array[Vector4]) -> void:
	...
	for gap in openings:
		if gap.x > cursor:
			_wall_box(axis, fixed, (cursor + gap.x) / 2, LEVEL + 1.7, gap.x - cursor, 3.4, "plaster", accent)
		if gap.z > 0:
			_wall_box(axis, fixed, (gap.x + gap.y) / 2, LEVEL + gap.z / 2, gap.y - gap.x, gap.z, "plaster", accent)
		_wall_box(axis, fixed, (gap.x + gap.y) / 2, LEVEL + (gap.w + 3.4) / 2, gap.y - gap.x, 3.4 - gap.w, "plaster", accent)
		...
		cursor = gap.y
	if cursor < width / 2:
		_wall_box(axis, fixed, (cursor + width / 2) / 2, LEVEL + 1.7, width / 2 - cursor, 3.4, "plaster", accent)
	_wall_box(axis, fixed, 0, LEVEL + 3.32, width + 0.08, 0.1, "metal", FRAME, false)
...
func _build() -> void:
	...
	for side in [-1, 1]:
		if side == outward:
			part(Vector3(side * 4.6, ROOF + 0.5, 1), Vector3(0.25, 1, 9), "plaster", accent)
		else:
			part(Vector3(side * 4.6, ROOF + 0.5, 0), Vector3(0.25, 1, 11.3), "plaster", accent)
		var height := 2.2 if side == signf(origin.z) else 1.0
		part(Vector3(0, ROOF + height / 2, side * 5.6), Vector3(9.4, height, 0.25), "plaster", accent)
	...
	part(Vector3(1, ROOF + 0.65, 0.3), Vector3(2.3, 1.3, 2.3), "metal", Color("87958f"))
	if batch:
		for x in [0.45, 1.55]:
		...
		arena.sign_text("01 / UPPER FLOOR", origin + Vector3(0, LEVEL + 2.6, -5.28), 0, TRIM, 30)
		arena.sign_text("ROOF ACCESS  →", origin + Vector3(-0.5, LEVEL + 2.2, -5.28), 0, TRIM, 24)
